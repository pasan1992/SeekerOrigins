...
public class EnvironmentSound : MonoBehaviour
{
    ...
    public void broadcastSound(Vector3 position, AgentBasicData.AgentFaction faction,float sountMaxDistance)
    {
        if(m_positionCallback != null)
        {
            m_positionCallback(position,faction,sountMaxDistance);
        }
    }
